function CineScript(typescript, timing, options, container) {
    ...
    var speed = typeof options['speed'] !== 'undefined' ? options['speed'] : 2;
...
trigger_cinescripts = function(event) {
    console.log(event);
    var cinescripts = $(event.currentSlide).find(".cinescript");
    if (cinescripts.length > 0) {
        console.log(cinescripts);
        for (var i=0; i<cinescripts.length; i++) {
            var cinescript = cinescripts[i];
            console.log($(cinescript).data("cinescript"));
        };
    }
};
